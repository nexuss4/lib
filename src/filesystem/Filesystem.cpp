...
namespace lce::fs {
    Filesystem::Filesystem() {
        this->root =
            std::make_unique<Directory>(ROOT, nullptr); // call the fs root "/"
    }

    Filesystem::Filesystem(fs::Directory *root) {
        root->setName(ROOT);

        this->root = std::unique_ptr<fs::Directory>(root);
    }
    ...
    Filesystem::getOrCreateDirByPath(const std::wstring &path) const {
        ...
        while (std::getline(ss, name, L'/')) { // go through the entire path
            ...
            if (!c || c->isFile())
                return nullptr; // say we want test/test2/test3.png, if test2 is
                                // file instead of dir then we're fucked.
            ...
            if (!child->isFile()) { // if it's not a file then we're going down
                                    // another directory in the tree, unless the
                                    // path ends in a directory
                c = dynamic_cast<Directory *>(child);
            } else { // somehow we found *a* file but aren't at the end of the
                     // path... we should not return it since the path is
                     // considered invalid
                return nullptr;
            }
        }
    ...
    FSObject *Filesystem::getByPath(const std::wstring &path) const {
        ...
        while (std::getline(ss, name, L'/')) { // go through the entire path
            ...
            if (!child->isFile()) { // if it's not a file then we're going down
                                    ...
                                    // path ends in a directory
                c = dynamic_cast<Directory *>(child);
            } else if (ss.eof()) { // we've found the file, and are at the end
                                   // of the path
                return child;
            } else { // somehow we found *a* file but aren't at the end of the
                     // path... we should not return it since the path is
